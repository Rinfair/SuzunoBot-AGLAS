...
async def _(bot: Bot, event: Event, message: Message = CommandArg()):
    help_str = '''▼ 系统设置帮助 | Commands For Suzuno System
------------------------------------------------------------------------------------------------------------------------------
以下设置，除显示系统状态外，都需要操作者的身份是群管理或者是铃乃管理员时这些操作才会执行。
部分高权限、高风险操作只有发送者的身份是铃乃管理员才可以执行，这些命令将在这里以[*]标注。

警告: 操作以[!]标注的这些设置会直接影响铃乃的稳定性，严禁随意使用。发生严重后果的， Suzuno 会将操
作者所在群退出并拉黑操作者。 若操作者为铃乃管理员，取消其管理权限。

这些操作，除系统状态外，都需要 @铃乃 才会执行。

[ ][ ] 系统状态 / ping              --  显示铃乃当前的系统状态。

[!][*] restart / 重启              --  在不重启计算机的情况下，重启铃乃服务。
注: 切勿很短时间内重启多次，会影响服务。

[!][*] 群发 / send              --  向铃乃所在的所有群发送消息，您不得使用此命令发送有害信息。
------------------------------------------------------------------------------------------------------------------------------'''
...
async def _(bot: Bot, event: Event):
    if str(event.user_id) not in Config.superuser:
        await restartbot.send("您还不是铃乃的管理员，无法重新启动铃乃。请联系管理员。")
        return
    else:
        flag = False
        if using_distro == True:
            ...
            if distro.name() == "" and distro.version() == "":
                flag = False
        if flag == False:
            await restartbot.send("正在重新启动 Suzuno Server......\n请稍候，重启操作需要1-3分钟。")
            restart()
        else:
...
async def _(bot: Bot, event: Event, message: Message = CommandArg()):
    if str(event.user_id) not in Config.superuser:
        await sendmsg.send("您还不是铃乃的管理员，无法给所有参与的群发送通知。请联系管理员。")
        return
    else:
